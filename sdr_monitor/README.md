...
When backend live config is enabled, fixed objects are merged: backend fixed objects are used as base, and local `fixed_objects` are added/replaced by matching `name`.
You can tune moving-object trail length with `trail_point_window_seconds`, set concrete symbol font sizes with `aircraft_symbol_font_px`, `vessel_symbol_font_px`, and `fixed_symbol_font_px`, and control zoom-dependent symbol scaling with `zoom_font_scale_factor`.
In the QT UI, use the `Installningar` button to open a dialog where you can save new defaults to `config.json` and apply a temporary moving-symbol font size for the current session.
